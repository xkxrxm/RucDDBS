...
enum class LogRecordType : std::int32_t {
    INVALID = 0,
    CREATE_TABLE,
    DROP_TABLE,
    INSERT,
...
static std::string log_record_type[15] = {"INVALID",
                                          "CREATE_TABLE",
                                          "DROP_TABLE",
                                          "INSERT",
                                          "DELETE",
...
public:
    ...
    LogRecord(txn_id_t txn_id, lsn_t prev_lsn, LogRecordType log_type)
        : size_(HEADER_SIZE),
          ...
          prev_lsn_(prev_lsn),
          log_type_(log_type) {
        assert(log_type == LogRecordType::BEGIN ||
               log_type == LogRecordType::COMMIT ||
               log_type == LogRecordType::ABORT ||
               log_type == LogRecordType::PREPARED);
    }
    ...
    LogRecord(txn_id_t txn_id,
        ...
        size_ = HEADER_SIZE + sizeof(int32_t) + key_size_ + sizeof(int32_t) +
                value_size_;
    }
    ...
    static LogRecord DeserializeFrom(const char *storage) {
        int size = *reinterpret_cast<const int32_t *>(storage);
        ...
        case LogRecordType::DELETE: {
            ...
            res = LogRecord(
                txn_id, prev_lsn, type, key_size, key, value_size, value);
            break;
        }
        default:
