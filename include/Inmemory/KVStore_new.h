...
public:
    ...
    bool del(const std::string &key,
        ...
        if (if_in_mem) {
            // if(enable_logging){
            //     //写Del日志
            //     LogRecord record (txn->get_txn_id(), txn->get_prev_lsn(),
            ...
            // //add write record into write set.
            // if(add_writeset){
            //     WriteRecord wr = WriteRecord(key, memtable_.get(key).second,
            //     WType::DELETE_TUPLE); txn->get_write_set()->push_back(wr);
            // }
            memtable_.del(key);
            return true;
        }
        ...
        if (result.first && result.second != "") {
            ...
            //     LogRecord record (txn->get_txn_id(), txn->get_prev_lsn(),
            //     LogRecordType::DELETE,
            //                 key.size(), key.c_str() , result.second.size(),
            //                 result.second.c_str());
            //     auto lsn = log_manager_->AppendLogRecord(record);
            //     txn->set_prev_lsn(lsn);
            // }
            // //add write record into write set.
            // if(add_writeset){
            //     WriteRecord wr = WriteRecord(key, result.second,
            //     WType::DELETE_TUPLE); txn->get_write_set()->push_back(wr);
            // }
            memtable_.put(key, "");
            return true;
