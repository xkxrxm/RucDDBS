...
namespace session {
void ConvertIntoPlan(const ChildPlan *child_plan,
                     std::shared_ptr<Operators> operators,
                     Context *context) {
    auto cur_op = operators;
    while (child_plan->child_plan_case() !=
           ChildPlan::ChildPlanCase::CHILD_PLAN_NOT_SET) {
        cout << "get - plan" << endl;
        ...
        case ChildPlan::ChildPlanCase::kFilterPlan: {
            ...
            auto lhs = make_shared<ast::Col>(exp.col().tab_name(),
                                             exp.col().col_name());
            auto op = ast::SvCompOp(exp.op());

            switch ((exp.exp().exp_case())) {
            case Expr::ExpCase::kCol: {
                auto tmp = make_shared<ast::Col>(exp.exp().col().col_name(),
                                                 exp.exp().col().tab_name());
        ...
        case ChildPlan::ChildPlanCase::kInsertPlan: {
            ...
            for (int i = 0; i < child_plan->insert_plan().col_name().size();
                 i++) {
        ...
        case ChildPlan::ChildPlanCase::kUpdatePlan: {
            ...
            for (int i = 0; i < size; i++) {
                ptr->set_col_index.push_back(
                    child_plan->update_plan().set_col_index()[i]);
...
public:
    Session_Service_Impl(TransactionManager *transaction_manager_sql) {
        transaction_manager_ = transaction_manager_sql;
    }
    ...
    virtual void SQL_Transfer(google::protobuf::RpcController *cntl_base,
        ...
        cout << request->sql_statement() << "txn_id = " << request->txn_id()
             << endl;
        txn_id_t txn_id = request->txn_id();
        Context *context = new Context(nullptr, transaction_manager_);
        try {
            auto sql_ret =
                Sql_execute_client(request->sql_statement(), txn_id, context);
            cout << "sql - over" << endl;
            cout << sql_ret << endl;
