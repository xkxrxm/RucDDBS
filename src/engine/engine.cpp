...
std::map<string, string> nodes_name{{"0.0.0.0:8005", "server1"},
                                    {"0.0.0.0:8006", "server2"}};
...
int get_res(const session::Table &response,
...
int send_plan(string target_address,
              shared_ptr<Operators> node_plan,
              ...
              txn_id_t txn_id)
{
    ...
    while (node_plan != nullptr)
    {
        ...
        else if (auto point_select =
                     dynamic_pointer_cast<op_point_select>(node_plan))
        {
        ...
        else if (auto insert_message =
                     dynamic_pointer_cast<op_insert>(node_plan))
        {
            // 实现insert到message构造
        ...
        else if (auto update_message =
                     dynamic_pointer_cast<op_update>(node_plan))
        {
        ...
        else if (auto delete_message =
                     dynamic_pointer_cast<op_delete>(node_plan))
        {
            // 实现delete到message构造
        ...
        else if (auto select_message =
                     dynamic_pointer_cast<op_selection>(node_plan))
        {
            // 实现select到message构造
            ...
            if (auto x =
                    dynamic_pointer_cast<ast::Col>(select_message->conds->rhs))
            {
            ...
            else if (auto x = dynamic_pointer_cast<ast::Value>(
                         select_message->conds->rhs))
            {
    ...
    if (response.abort())
        throw TransactionAbortException(txn_id,
                                        AbortReason::PARTICIPANT_RETURN_ABORT);
...
bool create_table(shared_ptr<ast::CreateTable> create_table)
{
    ...
    for (auto &field : create_table->fields)
    {
        if (auto parser_col_def = std::dynamic_pointer_cast<ast::ColDef>(field))
        {
            ...
            cols_info->column_type.push_back(
                ColType(int(parser_col_def->type_len->type)));
        }
        else
        {
...
void build_insert_plan(shared_ptr<ast::InsertStmt> insert_tree,
                       ...
                       Context *context)
{
    ...
    // 默认第一列为主键
    auto par_ips = GetTableIp(insert_plan->tab_name,
                              col_info.column_name[0],
                              to_string(iter->row[0]->str),
                              to_string(iter->row[0]->str));
...
void build_delete_plan(shared_ptr<ast::DeleteStmt> delete_tree,
                       std::shared_ptr<op_executor> exec_plan,
                       Context *context)
{
    ...
    // 默认第一列为主键
    auto par_ips = GetTableIp(
        tab_name, col_info.column_name[0], to_string(0), to_string(10000000));
    shared_ptr<op_distribution> distribution_plan(new op_distribution(context));
    ...
    for (auto iter : par_ips)
    {
        ...
        for (int i = 0; i < int(delete_tree->conds.size()); i++)
        {
            ...
            {
                shared_ptr<op_selection> selection_plan(
                    new op_selection(context));
...
void build_update_plan(shared_ptr<ast::UpdateStmt> update_tree,
                       std::shared_ptr<op_executor> exec_plan,
                       Context *context)
{
    shared_ptr<Operators> cur_node = exec_plan;
                       ...
                       Context *context)
{
    ...
    {
        std::shared_ptr<op_projection> projection_plan(
            new op_projection(context));
        for (auto iter : select_tree->cols)
        {
            shared_ptr<ast::Col> col(
                new ast::Col(iter->tab_name, iter->col_name));
    ...
    if (select_tree->tabs.size() > 1)
    {
        ...
        for (auto &tab : select_tree->tabs)
        {
            ...
            if (par_ips.empty())
            {
                LOG(ERROR) << "Fail to get partition location for table "
                           << tab;
                return;
            }
            // 构造分布查询
    ...
    else if (select_tree->tabs.size() == 1)
    {
        ...
        auto par_ips =
            GetTableIp(select_tree->tabs[0], col_info.column_name[0]);
        // 构造分布查询
        if (par_ips.empty())
        {
            LOG(ERROR) << "Fail to get partition location for table "
              ...
              string node)
{
    ...
    if (auto x = std::dynamic_pointer_cast<op_distribution>(tmp))
    {
        ...
        {
            prt_plan(x->nodes_plan[i],
                     res_txt,
                     offset,
                     nodes_name[x->target_address[i]]);
        }
    }
...
bool showPlan(std::shared_ptr<ast::Explain> explain_tree,
              vector<vector<string>> &res_txt,
              Context *context)
{
        ...
        if (ast::parse_tree != nullptr)
        {
            ...
            if (auto x = std::dynamic_pointer_cast<ast::ShowTables>(root))
            {
                showTables(res_txt);
                         ...
                         std::dynamic_pointer_cast<ast::ShowPartitions>(root))
            {
            ...
            else if (auto x = std::dynamic_pointer_cast<ast::SelectStmt>(root))
            {
                ...
                {
                    exec_plan->txn =
                        exec_plan->transaction_manager_->getTransaction(txn_id);
                }
            ...
            else if (auto x = std::dynamic_pointer_cast<ast::CreateTable>(
                         ast::parse_tree))
            {
                if (create_table(x))
                {
            ...
            else if (auto x = std::dynamic_pointer_cast<ast::DropTable>(
                         ast::parse_tree))
            {
                if (drop_table(x))
                {
