...
void RecoveryManager::Redo() {
    // currently, we only support recovery from empty database, so there is no
    // dirty page
    int offset = 0;
    ...
    while (log_storage_->ReadLog(buffer_, LOG_BUFFER_SIZE, offset)) {
        ...
        while (true) {
            ...
            uint32_t size =
                *reinterpret_cast<const uint32_t *>(buffer_ + inner_offset);
            // size = 0 means there is no more log records
            // we shall stop when buffer is empty or there is no more log
            // records
            ...
            lsn_mapping_[log.GetLsn()] =
                std::make_pair(offset + inner_offset, size);
...
void RecoveryManager::Undo() {
    ...
    // at every step in undo phase, we need to execute the log record with max
    // lsn in undo-list
...
void RecoveryManager::UndoLog(LogRecord &log_record) {
    ...
    case LogRecordType::PREPARED:
        ...
        break;
    case LogRecordType::INSERT: {
        // 这里调用Kvstore不传入txn事务, 因为在恢复过程中没有维护事务管理器,
        // 通过active_txn_来跟踪 每个事务最后的lsn, 写入存储层后,
        // 在本函数中通过active_txn_维护写入的日志信息
        kv_->del(std::string(log_record.GetKey(), log_record.GetKeySize()));
        // undo操作也需要写redo log
        auto log = LogRecord(log_record.GetTxnId(),
                             active_txn_[log_record.GetTxnId()],
                             LogRecordType::DELETE,
                             log_record.GetKeySize(),
                             log_record.GetKey(),
                             log_record.GetValueSize(),
                             log_record.GetValue());
        log_manager_->AppendLogRecord(log);
